...
const Create = ({ marketplace, nft }) => {
    ...
    const uploadToIPFS = async (event) => {
        ...
        if (typeof file !== 'undefined') {
            try {
                ...
                console.log(result);
                setImage(`https://nftmarketplace-zebermvp.infura-ipfs.io/ipfs/${result.path}`);
            } catch (error) {
    ...
    const createNFT = async () => {
        ...
        try {
            ...
            console.log("IPFS uri load error ", error);
        }
    }

    const mintThenList = async (result) => {
        const uri = `https://nftmarketplace-zebermvp.infura-ipfs.io/ipfs/${result.path}`;
